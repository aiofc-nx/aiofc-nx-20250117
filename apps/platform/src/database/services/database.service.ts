...
export class DatabaseService implements OnModuleInit {
   ...
   */
  constructor(private env: EnvOption) {
  ...
  async onModuleInit() {
    ...
    await this.client`
      ALTER DATABASE ${this.client(this.env.database.name)} SET row_security = on;
    `;
  ...
  async getTenantDB(tenantId: string) {
    return await this.client.begin(async (transaction) => {
      await transaction`SET LOCAL app.current_tenant_id = ${transaction(tenantId)}`;
  ...
  async setTenantContext(tenantId: string) {
    await this
      .client`SET LOCAL app.current_tenant_id = ${this.client(tenantId)}`;
  }
